...
async def main() -> int:
    ...
    config: RunnableConfig = {"configurable": {"thread_id": f"retrieval-audit-{generate_uuid()[:12]}"}}
    ...
    _write(artifact_dir / "scope_config.json", scope_payload(get_scope_config()))
    ...
    print(f"  scope mode: {'demo' if get_scope_config().demo_scope_enabled else 'full'}")
    print(f"  companies: {get_scope_config().companies}")
